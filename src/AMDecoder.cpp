...
struct AMDecoder : Module {
    ...
    AMDecoder() {
		...
		configParam(AMDecoder::CH1_ATTACK_PARAM, 0.0, 1.0, 0.1, "");
		configParam(AMDecoder::CH1_RELEASE_PARAM, 0.0, 1.0, 0.3, "");
		configParam(AMDecoder::CH1_ATTENUVERTER_PARAM, -1.0, 1.0, 1.0, "");
		configParam(AMDecoder::CH1_OFFSET_PARAM, -10.0, 10.0, 0.0, "");
		configParam(AMDecoder::CH1_SCALE_PARAM, 1.0, 3.0, 1.0, "");
		configParam(AMDecoder::CH2_ATTACK_PARAM, 0.0, 1.0, 0.1, "");
		configParam(AMDecoder::CH2_RELEASE_PARAM, 0.0, 1.0, 0.3, "");
		configParam(AMDecoder::CH2_ATTENUVERTER_PARAM, -1.0, 1.0, 1.0, "");
		configParam(AMDecoder::CH2_OFFSET_PARAM, -10.0, 10.0, 0.0, "");
		configParam(AMDecoder::CH2_SCALE_PARAM, 1.0, 3.0, 1.0, "");
	}
...
void AMDecoder::process(const ProcessArgs& args) {
...
struct AMDecoderWidget : ModuleWidget {
	AMDecoderWidget(AMDecoder *module) {
...

		{
			...
			panel->setBackground(APP->window->loadSvg(asset::plugin(pluginInstance, "res/AMDecoder.svg")));
			addChild(panel);
		}
